...
<div class="content_wrapper">
	...
	<tr>
		...
		<th >納品予定日</th>
		<td>
			<input type="date" class="form-control search_box" value="{{$order->delivery_date}}">(○日後)</td>
	</tr>
	<tr>
		<th >納品状態</th>
		<td>{{$order->status}}</td>
	...
	<div class="controll_buttons overflow_btn">
		...
		<input type="submit" class="btn btn-dark" value="発注情報変更">
	</div>
	<table class="table-bordered">
		<tr>
		...
		@foreach($order_contents as $content)
			<tr>
				<form action="/order/content/update" method="POST">
					...
					<td>{{$content->arrival_date or '未完了'}}</td>
					...
					<td  class="btn-outer">
						@if($content->is_arrival >= $content->amount)
							<button class="btn" disabled type="button">入荷</button>
						@else
